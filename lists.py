...
def long_words(words):
    ...
    longer_than_four_list = []
    for word in words:
        if len(word) > 4:
            longer_than_four_list.append(word)
    print(longer_than_four_list)
...
def halvesies(numbers):
    ...
    for given_number in numbers:
        halved_list.append(given_number/2)
    return halved_list
...
def duplicates(items):
    ...
    for i in range(1, len(items)):
        if items[i] == items[i -1]:
                if items[i] not in duplicate_items:
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for w in words:
        ...
        letter_index.append(success_index)
    return letter_index
